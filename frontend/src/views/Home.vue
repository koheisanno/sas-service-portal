        ...
        <div class="col-sm-6 mb-3">
          <div class="border rounded bg-white p-4">
            ...
            <ul v-else class="list-group list-container">
              <li v-for="event in upcomingEvents.slice(0,10)" :key="event.id" :set="club = findClubData(event.club)" class="list-group-item">
                <div>
                  <p style="display:flex; justify-content:space-between;">
                    <span class="fw-bold text-dark">{{ event.name }}</span>
                    <span v-if="event.is_meeting" class="badge bg-secondary">Meeting</span>
                    <span v-else class="badge bg-primary">Event</span>
                  </p>
